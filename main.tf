...
// Create a VPC named "edlarios_vpc"
resource "aws_vpc" "edlarios_vpc" {
  ...
  // We are tagging the VPC with the name "edlarios_vpc"
  tags = {
    Name = "edlarios_vpc"
  }
}

// Create an internet gateway named "edlarios_igw"
// and attach it to the "edlarios_vpc" VPC
resource "aws_internet_gateway" "edlarios_igw" {
  // Here we are attaching the IGW to the 
  // edlarios_vpc VPC
  vpc_id = aws_vpc.edlarios_vpc.id

  // We are tagging the IGW with the name edlarios_igw
  tags = {
    Name = "edlarios_igw"
  }
}
...
resource "aws_subnet" "edlarios_public_subnet" {
  ...
  // Put the subnet into the "edlarios_vpc" VPC
  vpc_id = aws_vpc.edlarios_vpc.id
  ...
  // We are tagging the subnet with a name of "edlarios_public_subnet_" and
...
resource "aws_subnet" "edlarios_private_subnet" {
  // count is the number of resources we want to create
  ...
  // We are tagging the subnet with a name of "edlarios_private_subnet_" and
  // suffixed with the count
  tags = {
    Name = "edlarios_private_subnet_${count.index}"
  }
}

// Create a public route table named "edlarios_public_rt"
resource "aws_route_table" "edlarios_public_rt" {
  // Put the route table in the "edlarios_vpc" VPC
  vpc_id = aws_vpc.edlarios_vpc.id
  ...
  // Gateway "edlarios_igw"
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.edlarios_igw.id
  }
}

// Here we are going to add the public subnets to the 
// "edlarios_public_rt" route table
resource "aws_route_table_association" "public" {
  ...
  // "edlarios_public_rt" from above
  route_table_id = aws_route_table.edlarios_public_rt.id

  // This is the subnet ID. Since the "edlarios_public_subnet" is a 
  // list of the public subnets, we need to use count to grab the
  // subnet element and then grab the id of that subnet
  subnet_id = aws_subnet.edlarios_public_subnet[count.index].id
}

// Create a private route table named "edlarios_private_rt"
resource "aws_route_table" "edlarios_private_rt" {
  // Put the route table in the "edlarios_VPC" VPC
  vpc_id = aws_vpc.edlarios_vpc.id
...
// route table "edlarios_private_rt"
resource "aws_route_table_association" "private" {
  ...
  // Here we are making sure that the route table is
  // "edlarios_private_rt" from above
  route_table_id = aws_route_table.edlarios_private_rt.id

  // This is the subnet ID. Since the "edlarios_private_subnet" is a
  // list of private subnets, we need to use count to grab the
  // subnet element and then grab the ID of that subnet
  subnet_id = aws_subnet.edlarios_private_subnet[count.index].id
}

// Create a security for the EC2 instances called "edlarios_web_sg"
resource "aws_security_group" "edlarios_web_sg" {
  // Basic details like the name and description of the SG
  name        = "edlarios_web_sg"
  description = "Security group for edlarios web servers"
  // We want the SG to be in the "edlarios_vpc" VPC
  vpc_id = aws_vpc.edlarios_vpc.id
  ...
  // Here we are tagging the SG with the name "edlarios_web_sg"
  tags = {
    Name = "edlarios_web_sg"
  }
}

// Create a security group for the RDS instances called "edlarios_db_sg"
resource "aws_security_group" "edlarios_db_sg" {
  // Basic details like the name and description of the SG
  name        = "edlarios_db_sg"
  description = "Security group for edlarios databases"
  // We want the SG to be in the "edlarios_vpc" VPC
  vpc_id = aws_vpc.edlarios_vpc.id
  ...
  ingress {
    ...
    to_port         = "3306"
    protocol        = "tcp"
    security_groups = [aws_security_group.edlarios_web_sg.id]
  }

  // Here we are tagging the SG with the name "edlarios_db_sg"
  tags = {
    Name = "edlarios_db_sg"
  }
}

// Create a db subnet group named "edlarios_db_subnet_group"
resource "aws_db_subnet_group" "edlarios_db_subnet_group" {
  // The name and description of the db subnet group
  name        = "edlarios_db_subnet_group"
  description = "DB subnet group for edlarios"

  // Since the db subnet group requires 2 or more subnets, we are going to
  // loop through our private subnets in "edlarios_private_subnet" and
  // add them to this db subnet group
  subnet_ids = [for subnet in aws_subnet.edlarios_private_subnet : subnet.id]
}

// Create a DB instance called "edlarios_database"
resource "aws_db_instance" "edlarios_database" {
  ...
  // settings.database.db_name variable, which is set to "edlarios"
  ...
  // This is the DB subnet group "edlarios_db_subnet_group"
  db_subnet_group_name = aws_db_subnet_group.edlarios_db_subnet_group.id
  ...
  // "edlarios_db_sg" security group
  vpc_security_group_ids = [aws_security_group.edlarios_db_sg.id]
...
// Create a key pair named "edlarios_kp"
resource "aws_key_pair" "edlarios_kp" {
  // Give the key pair a name
  key_name = "edlarios_kp"
...
resource "aws_instance" "edlarios_web" {
  ...
  // The subnet ID for the EC2 instance. Since "edlarios_public_subnet" is a list
  ...
  // "edlarios_public_subnet" and putting the EC2 instance in there
  subnet_id = aws_subnet.edlarios_public_subnet[count.index].id

  // The key pair to connect to the EC2 instance. We are using the "edlarios_kp" key 
  // pair that we created
  key_name = aws_key_pair.edlarios_kp.key_name
 ...
 connection {
  ...
  // the count index
  tags = {
    Name = "edlarios_web_${count.index}"
  }
}

// Create an Elastic IP named "edlarios_web_eip" for each
// EC2 instance
resource "aws_eip" "edlarios_web_eip" {
  ...
  // The EC2 instance. Since edlarios_web is a list of 
  ...
  instance = aws_instance.edlarios_web[count.index].id

  // We want the Elastic IP to be in the VPC
  vpc = true

  // Here we are tagging the Elastic IP with the name
  // "edlarios_web_eip_" followed by the count index
  tags = {
    Name = "edlarios_web_eip_${count.index}"
  }
}
